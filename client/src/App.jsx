...
/* =====================
   Coach Client View
===================== */
function CoachClientView({ token, unit, onInvalidToken, onError }) {
  ...
  async function loadClientData(clientId) {
    try {
      ...
      const [weekly, daily, adherence, progress] = await Promise.all([
        ...
        apiFetch(`/api/coach/clients/${clientId}/adherence?from=${isoLocalNDaysAgo(13)}&to=${to}`, { token, onInvalidToken }).catch(() => null),
  ...
  return (
    <div className="card" style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        {selectedClientId && (
          <button className="secondary" onClick={() => loadClientData(selectedClientId)} disabled={busy}>
            {busy ? "…" : "Refresh"}
          </button>
        )}
        {selectedClientId && (
          <button className="secondary" onClick={() => { setSelectedClientId(""); setClientData(null); }}>
            Clear
          </button>
        )}
      </div>
      ...
      {selectedClient && clientData && (
        <>
              ...
              {clientData.progress.progress_pct != null && (
                <>
                  <div className="small" style={{ marginTop: 6 }}>
                    Progress: <b>{clientData.progress.progress_pct.toFixed(0)}%</b>
                  </div>
                  <div style={{ width: "100%", height: 8, background: "rgba(255,255,255,0.08)", borderRadius: 999, marginTop: 6 }}>
                    <div style={{ width: `${clientData.progress.progress_pct}%`, height: 8, borderRadius: 999, background: "rgba(239,68,68,0.9)" }} />
                  </div>
                </>
              )}
            </div>
          ...
          {/* Adherence */}
          {clientData.adherence && clientData.adherence.planned_sessions > 0 && (
            <div className="grid grid-3" style={{ marginBottom: 14 }}>
              <div className="metric">
                <div className="k">Planned (14d)</div>
                <div className="v">{clientData.adherence.planned_sessions}</div>
              </div>
              <div className="metric">
                <div className="k">Completed</div>
                <div className="v">{clientData.adherence.completed_sessions}</div>
              </div>
              <div className="metric">
                <div className="k">Adherence</div>
                <div className="v">
                  {clientData.adherence.adherence_pct != null
                    ? `${clientData.adherence.adherence_pct.toFixed(1)}%`
                    : "—"}
                </div>
              </div>
            </div>
            ...
            {(["Bench", "Squat", "Deadlift"]).map((ex) => {
              const best = Math.max(
                ...
                ...((clientData.daily || []).flatMap((d) =>
                  (d?.entries || [])
                    .filter((e) => normalizeExerciseName(e?.exercise) === normalizeExerciseName(ex))
              ...
              return (
                <div className="metric" key={ex}>
                  ...
                  <div className="v">{Number.isFinite(best) ? `${fmt(best)} ${unit}` : "—"}</div>
                </div>
              );
            })}
          </div>
                        ...
                        <td>
                          {(w.entries || []).map((e, i) => (
                            <div key={i} className="small">
                              {e.exercise}: <b>{fmt(e.top)} × {e.reps}</b>
                              {e.rpe ? ` @ RPE ${e.rpe}` : ""}
                            </div>
                          ))}
                        </td>
            ...
            {dailyEntry ? (
              <div style={{ marginTop: 12 }}>
                ...
                <div className="grid grid-3" style={{ marginBottom: 10 }}>
                  {dailyEntry.bodyweight && (
                    <div className="metric"><div className="k">Bodyweight</div><div className="v">{dailyEntry.bodyweight} {unit}</div></div>
                  )}
                  {dailyEntry.sleep_hours && (
                    <div className="metric"><div className="k">Sleep</div><div className="v">{dailyEntry.sleep_hours}h</div></div>
                  )}
                  {dailyEntry.zone2_mins && (
                    <div className="metric"><div className="k">Zone2</div><div className="v">{dailyEntry.zone2_mins} mins</div></div>
                  )}
                </div>
                {dailyEntry.notes && (
                  <div className="small" style={{ marginBottom: 10 }}>Notes: {dailyEntry.notes}</div>
                )}
                <div className="list">
                  {(dailyEntry.entries || []).map((e, i) => (
                    <div className="listRow" key={i} style={{ alignItems: "flex-start" }}>
                      <div>
                        ...
                        {e?.planned?.sets_reps && (
                          <div className="small">
                            Plan: <b>{e.planned.sets_reps}</b> {e.planned.load_rpe ? `@ ${e.planned.load_rpe}` : ""}
                      ...
                      <div>
                        {e.completed
                          ? <span className="pill" style={{ borderColor: "rgba(16,185,129,0.5)", background: "rgba(16,185,129,0.12)" }}>✓</span>
...
/* =====================
   Overview
===================== */
function Overview({ me, token, unit, tracked, dashboardExercises, weekly, dailyOverview, activeProgram, onLogout, onRefresh, onInvalidToken, onError }) {
  return (
    <>
    {/* Coach client switcher — only renders if user has clients */}
      <CoachClientView
        token={token}
        unit={unit}
