...
// Verify coach→client relationship before any data access
...
// Client weekly entries
app.get("/api/coach/clients/:clientId/weekly", requireAuth, async (req, res) => {
  try {
...
// Client daily entries range
app.get("/api/coach/clients/:clientId/daily", requireAuth, async (req, res) => {
  try {
    const { clientId } = req.params;
    if (!await requireCoachAccess(req.user.id, clientId)) {
      return res.status(403).json({ error: "Not authorised to view this client" });
    ...
    const q = await pool.query(
      `select entry_date, unit, bodyweight, sleep_hours, pec_pain_0_10,
              zone2_mins, notes, entries, is_completed, completed_at, created_at, updated_at
       from public.daily_entries_app
       where user_id = $1
         and entry_date between $2::date and $3::date
       order by entry_date asc`,
...
// Client single daily entry
app.get("/api/coach/clients/:clientId/daily/:date", requireAuth, async (req, res) => {
  try {
...
// Client adherence
app.get("/api/coach/clients/:clientId/adherence", requireAuth, async (req, res) => {
  try {
    const { clientId } = req.params;
    if (!await requireCoachAccess(req.user.id, clientId)) {
      return res.status(403).json({ error: "Not authorised to view this client" });
    ...
    for (const iso of dates) {
      ...
      const dayRow = dailyByDate.get(iso);
      const isCompleted = dayRow ? isDayCompleted(dayRow) : false;
      if (isCompleted) completed++;
...
// Client active program progress
app.get("/api/coach/clients/:clientId/progress", requireAuth, async (req, res) => {
  try {
    const { clientId } = req.params;
    if (!await requireCoachAccess(req.user.id, clientId)) {
      return res.status(403).json({ error: "Not authorised to view this client" });
